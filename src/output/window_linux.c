...
static const char* create_wayland_window(lx_window* window)
{
    ...
    produce_debug("-> created wl display");
    ...
    produce_debug("-> created wl registry");
...
static const char* create_xdg_shell(lx_window* window)
{
    ...
    produce_debug("-> created xdg wm base");
    ...
    produce_debug("-> created xdg surface");
    ...
    produce_debug("-> created xdg toplevel");
    ...
    if (window->xdg_deco) // set by wl_display_roundtrip and the registry_global callback
    {
        ...
        produce_debug("-> created and using server side decorations");
    }
    else
        produce_debug("-> server side decorations unavailable, defaulting to client side decoration");
...
static void destroy_xdg_shell(lx_window* window)
{
    if (window->xdg_toplevel != NULL)
    {
        ...
        produce_debug("-> destroyed xdg toplevel");
    }

    if (window->xdg_surface != NULL)
    {
        xdg_surface_destroy(window->xdg_surface);
        produce_debug("-> destroyed xdg surface");
    }

    if (window->xdg_wm_base != NULL)
    {
        xdg_wm_base_destroy(window->xdg_wm_base);
        produce_debug("-> destroyed xdg wm base");
    }
}
...
static const char* create_egl_surface(lx_window* window)
{
    ...
    produce_debug("-> created egl context");
    ...
    produce_debug("-> created egl window"); 
    ...
    produce_debug("-> created egl surface");
...
{
    // set these first otherwise any errors will just be thrown away
    set_err_cb(properties.on_error);
    set_dbg_cb(properties.on_debug);
    ...
    produce_debug("creating window %s @ %dx%d", properties.title, properties.width, properties.height);

    lx_window* window = malloc(sizeof(lx_window));
    if (!window)
    {
        produce_error("failed to allocate window struct");
        return NULL;
    }
    produce_debug("-> allocated window struct");
    ...
    produce_debug("-> made window current");
    ...
    produce_debug("-> loaded opengl version %d.%d", (version / 10000), (version % 10000));

    produce_debug("finished creating window");
...
void lx_window_destroy(lx_window* window)
{
    ...
    produce_debug("destroying window %s", window->title);

    unload_gl_procs();
    produce_debug("-> unloaded opengl");
    ...
    free(window);
    produce_debug("-> freed window struct");
    
    internal_ref = NULL;
    produce_debug("finished destroying window");
